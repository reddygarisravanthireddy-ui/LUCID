 ...
 */
/**
 * Returns true when the raw text contains strong malicious-behavior indicators
 * that must override any documentation / test-document / defanged downgrade.
 *
 * Covers:
 *   - Invoke-WebRequest / DownloadString followed by IEX / Invoke-Expression
 *   - powershell -enc / encoded PowerShell
 *   - credential-harvesting URL patterns
 *   - phishing content requesting passwords or MFA codes
 *   - malware downloader / C2 staging patterns
 *
 * Does NOT fire on:
 *   - Benign documentation that merely mentions security concepts
 *   - Architecture diagrams, deployment guides, API references
 */
function hasMaliciousBehaviorOverride(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const t = rawText.toLowerCase();

  // 1. PowerShell download-and-execute chains (defanged or not)
  const hasPsDownload = (
    t.includes('invoke-webrequest') ||
    t.includes('downloadstring') ||
    t.includes('downloadfile') ||
    t.includes('net.webclient') ||
    t.includes('webclient')
  );
  const hasPsExecute = (
    t.includes('iex ') ||
    t.includes('iex(') ||
    t.includes('invoke-expression') ||
    t.includes('invoke-command') ||
    t.includes('start-process')
  );
  if (hasPsDownload && hasPsExecute) return true;

  // 2. Encoded PowerShell
  if (/powershell[^\n]{0,60}-enc/i.test(rawText)) return true;
  if (/powershell[^\n]{0,60}-encodedcommand/i.test(rawText)) return true;
  if (/\-enc\s+[A-Za-z0-9+/]{20,}/.test(rawText)) return true;

  // 3. Scheduled-task / persistence + downloader combo
  if (
    (t.includes('schtasks') || t.includes('at.exe') || t.includes('cron')) &&
    (hasPsDownload || t.includes('curl') || t.includes('wget'))
  ) return true;

  // 4. Credential-harvesting form patterns
  const hasHarvestingForm = (
    (t.includes('input type="password"') || t.includes("input type='password'") ||
     t.includes('name="password"') || t.includes("name='password'")) &&
    (t.includes('phish') || t.includes('harvest') || t.includes('steal') ||
     t.includes('evil') || t.includes('attacker') || t.includes('malicious'))
  );
  if (hasHarvestingForm) return true;

  // 5. Explicit phishing message asking for credentials/MFA
  const hasPhishingAsk = (
    (t.includes('enter your password') || t.includes('verify your password') ||
     t.includes('confirm your password') || t.includes('enter your mfa') ||
     t.includes('provide your otp') || t.includes('enter the code sent to')) &&
    (t.includes('click here') || t.includes('http://') || t.includes('https://') ||
     t.includes('verify now') || t.includes('act immediately') || t.includes('urgent'))
  );
  if (hasPhishingAsk) return true;

  // 6. Explicit malware / C2 staging language (combined with download)
  const hasMalwareKeywords = (
    (t.includes('c2') || t.includes('command and control') || t.includes('command-and-control') ||
     t.includes('dropper') || t.includes('payload') || t.includes('backdoor') ||
     t.includes('reverse shell') || t.includes('meterpreter')) &&
    (hasPsDownload || t.includes('curl') || t.includes('wget') || t.includes('http://'))
  );
  if (hasMalwareKeywords) return true;
...
async function analyzeUploadedFile(file, options = {}) {
  ...
  // If documentation (and no strong malicious-behavior signals), extract section findings
  const maliciousOverride = hasMaliciousBehaviorOverride(extracted.rawText || '');
  if (isDoc && !maliciousOverride && !staticFindings.some(f => f.severity === 'Critical')) {
  ...
  // Documentation downgrade is BLOCKED when strong malicious-behavior indicators are present.
  // This ensures defanged/test documents containing real attack patterns are not silenced.
  if (!maliciousOverride && hasBenignReasoning && (isContradictoryVerdict || isContradictoryHigh)) {
    classification = 'Documentation';
    severity = 'Low';
    verdict = 'SAFE';
    confidence = 'High';
    maliciousContent = 'not_detected';
  } else if (!maliciousOverride && isDoc && !hasCriticalStatic && !hasHighStatic) {
    classification = 'Documentation';
    severity = 'Low';
    verdict = 'SAFE';
    confidence = 'High';
    maliciousContent = 'not_detected';
  } else if (maliciousOverride && (verdict === 'SAFE' || verdict === 'INCONCLUSIVE')) {
    // Static scan caught the pattern but AI returned SAFE (e.g. due to doc framing).
    // Preserve as SUSPICIOUS so the threat is not silently dropped.
    verdict = 'SUSPICIOUS';
    maliciousContent = 'not_detected'; // static-analysis only — do not claim execution
