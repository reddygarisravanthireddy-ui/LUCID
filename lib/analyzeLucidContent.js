...
// ============================================================
// DOCUMENTATION DETECTION & PROMPT GUIDANCE
// Content-based detection only; never relies on filenames.
// ============================================================
const DOCUMENTATION_INSTRUCTIONS = `The supplied content appears to be software documentation.

Treat architecture diagrams, markdown,
configuration examples, HTML snippets,
login examples,
URLs,
deployment guides,
sample code,
screenshots,
forms,
and implementation examples as benign unless there is direct evidence of malicious behaviour.

Do not classify documentation as phishing,
credential harvesting,
malware,
or dangerous simply because it discusses security concepts.`;

/**
 * Lightweight documentation detector using content features only.
 * Checks for Markdown headings and key software documentation terms.
 * Does NOT detect using filename.
 */
function detectDocumentation(content) {
  if (!content || typeof content !== 'string') {
    return { isDocumentation: false, confidence: 'Low' };
  }
  ...
  // 1. Markdown headings: '# Heading', '## Heading', or setext underline headings
  const hasMarkdownHeadings = (
    /(?:^|\n)#{1,6}\s+\S+/m.test(text) ||
    /(?:^|\n)[^\n]+\n[=-]{3,}(?:\n|$)/m.test(text)
  );

  // 2. Target keywords specified in requirements
  const keywords = [
    { name: 'README', regex: /\breadme\b/i },
    { name: 'PROJECT OVERVIEW', regex: /\bproject\s+overview\b/i },
    { name: 'USER GUIDE', regex: /\buser\s+guide\b/i },
    { name: 'ARCHITECTURE', regex: /\barchitecture\b/i },
    { name: 'INSTALLATION', regex: /\binstallation\b/i },
    { name: 'DEPLOYMENT', regex: /\bdeployment\b/i },
    { name: 'TABLE OF CONTENTS', regex: /\btable\s+of\s+contents\b/i },
    { name: 'LICENSE', regex: /\blicense\b/i },
    { name: 'Vertex AI', regex: /\bvertex\s+ai\b/i },
    { name: 'Cloud Run', regex: /\bcloud\s+run\b/i },
    { name: 'Firebase', regex: /\bfirebase\b/i },
    { name: 'MITRE', regex: /\bmitre\b/i },
    { name: 'Node.js', regex: /\bnode(?:\.js)?\b/i },
    { name: 'Express', regex: /\bexpress\b/i },
    { name: 'API', regex: /\bapi\b/i }
  ];
  ...
  // Additional structural documentation markers
  const hasDocStructure = (
    /\brunbook\b/i.test(text) ||
    /\btechnical\s+handbook\b/i.test(text) ||
    /\bdocumentation\s+suite\b/i.test(text) ||
    /\bcontents\b/i.test(text) ||
    /\bgetting\s+started\b/i.test(text)
  );

  // Detection logic based solely on document content
  const isDoc = (
    (hasMarkdownHeadings && (matchedKeywords >= 1 || hasDocStructure)) ||
    matchedKeywords >= 2 ||
    (hasDocStructure && (matchedKeywords >= 1 || hasMarkdownHeadings))
  );

  let confidence = 'Low';
  if (isDoc) {
    if ((hasMarkdownHeadings && matchedKeywords >= 3) || matchedKeywords >= 4) {
      confidence = 'High';
    } else if (matchedKeywords >= 2 || (hasMarkdownHeadings && matchedKeywords >= 1)) {
      confidence = 'Medium';
    }
  }

  return {
    isDocumentation: Boolean(isDoc),
    confidence
  };
}
...
function applyEvidenceConsistency(result, rawText, mode) {
  ...
  const docDetection = detectDocumentation(rawText || '');

  // Strong malicious-behavior indicators that must override any documentation downgrade.
  // Defanged samples and test documents containing real attack patterns are not exempted.
  const rawTextStr = String(rawText || '');
  const tLow = rawTextStr.toLowerCase();

  const _hasPsDownload = (
    tLow.includes('invoke-webrequest') || tLow.includes('downloadstring') ||
    tLow.includes('downloadfile') || tLow.includes('net.webclient') || tLow.includes('webclient')
  );
  const _hasPsExecute = (
    tLow.includes('iex ') || tLow.includes('iex(') ||
    tLow.includes('invoke-expression') || tLow.includes('invoke-command')
  );
  const hasMaliciousOverride = (
    (_hasPsDownload && _hasPsExecute) ||
    /powershell[^\n]{0,60}-enc/i.test(rawTextStr) ||
    /\-enc\s+[A-Za-z0-9+/]{20,}/.test(rawTextStr) ||
    (
      (tLow.includes('c2') || tLow.includes('dropper') || tLow.includes('payload') ||
       tLow.includes('backdoor') || tLow.includes('reverse shell')) &&
      (_hasPsDownload || tLow.includes('curl') || tLow.includes('wget') || tLow.includes('http://'))
    )
  );

  // 0. CONSISTENCY VALIDATION & BENIGN DOCUMENTATION DOWNGRADE
  // Downgrade is BLOCKED when strong malicious-behavior indicators are present.
  ...
  const hasBenignReasoningPhrase = (
    reasoningFields.includes('no malicious indicators') ||
    reasoningFields.includes('benign documentation') ||
    reasoningFields.includes('standard project documentation') ||
    reasoningFields.includes('no evidence of phishing')
  );

  const isHighOrCritical = (
    String(result.severity || '').toLowerCase() === 'high' ||
    String(result.severity || '').toLowerCase() === 'critical'
  );

  const isDangerous = (
    String(result.verdict || '').toLowerCase() === 'dangerous'
  );

  if (!hasMaliciousOverride && hasBenignReasoningPhrase && (isDangerous || isHighOrCritical)) {
    result.classification = 'Documentation';
    result.severity = 'Low';
    result.verdict = 'Safe';
    result.confidence = 'High';
    result.mitre_attack = [];
    result.attack_chain = 'Single-stage event (Benign Documentation)';
    result.key_indicators = ['No malicious indicators', 'Benign software documentation'];
    result.soc_actions = ['No action required; this documentation content is benign.'];
    result.recommended_action = 'No action required; this documentation content is benign.';
    result.analysis_scope = "This document was analysed as static documentation.\nThe analysis examined readable content only.\nDocumentation may reference security concepts,\nlogin pages,\nor attack terminology without being malicious.";
    return result;
  }
  ...
  // 3B. ANALYST-ONLY: APPLICATION / API VULNERABILITY TAXONOMY
  // Specific demonstrated vulnerability families take precedence over generic phishing,
  // command injection, data-exfiltration, and generic RCE labels.
  if (mode === 'analyst') {
    // SSRF: user-controlled server-side fetch reaches an internal/metadata target and returns data.
    const hasSsrfTarget = (
      text.includes('169.254.169.254') || text.includes('metadata.google.internal') ||
      text.includes('/latest/meta-data') || text.includes('instance metadata') || text.includes('metadata endpoint')
    );
    const hasServerSideFetch = (
      text.includes('server-side') || text.includes('url fetch') || text.includes('fetch feature') ||
      text.includes('application successfully requested') || text.includes('application successfully fetched') ||
      text.includes('application into fetching') || text.includes('server fetched')
    );
    const hasSsrfSuccess = (
      text.includes('returned cloud') || text.includes('returned instance metadata') ||
      text.includes('metadata was returned') || text.includes('credentials were returned') ||
      text.includes('returned') && text.includes('metadata')
    );
    const hasSsrfNegation = (
      text.includes('ssrf was blocked') || text.includes('request was blocked') ||
      text.includes('metadata request was blocked') || text.includes('did not fetch') ||
      text.includes('no metadata was returned')
    );
    if (hasSsrfTarget && hasServerSideFetch && hasSsrfSuccess && !hasSsrfNegation) {
      result.verdict = 'Dangerous';
      result.classification = 'Server-Side Request Forgery (SSRF)';
      result.severity = (text.includes('credential') || text.includes('security-credentials')) ? 'Critical' : 'High';
      result.mitre_attack = ['T1190 — Exploit Public-Facing Application'];
      return result;
    }

    // Path traversal / arbitrary local file read: traversal syntax reaches a local file without OS command execution.
    const hasTraversalSyntax = (
      text.includes('../') || text.includes('..\\\\') || text.includes('%2e%2e%2f') || text.includes('%252e%252e')
    );
    const hasFileReadContext = (
      text.includes('file=') || text.includes('download?') || text.includes('path traversal') ||
      text.includes('local file') || text.includes('/etc/passwd') || text.includes('/etc/shadow')
    );
    const hasTraversalSuccess = (
      text.includes('returned contents') || text.includes('contents of') || text.includes('file contents') ||
      text.includes('returned in the http response') || text.includes('read the file')
    );
    const hasTraversalNegation = (
      text.includes('path traversal was blocked') || text.includes('traversal attempt was blocked') ||
      text.includes('request returned 403') || text.includes('request was rejected') ||
      text.includes('file was not returned')
    );
    if (hasTraversalSyntax && hasFileReadContext && hasTraversalSuccess && !hasTraversalNegation) {
      result.verdict = 'Dangerous';
      result.classification = 'Path Traversal / Arbitrary File Read';
      result.severity = 'High';
      result.mitre_attack = ['T1190 — Exploit Public-Facing Application'];
      return result;
    }

    // Stored XSS: attacker-controlled script is persisted and actually executes in another user's browser.
    const hasScriptPayload = text.includes('<script') || text.includes('javascript:') || text.includes('onerror=') || text.includes('onload=');
    const hasStoredXssContext = text.includes('stored') || text.includes('profile') || text.includes('comment') || text.includes('biography');
    const hasBrowserExecution = (
      text.includes('script executed') || text.includes('executed in') && text.includes('browser') ||
      text.includes('when another') && text.includes('viewed') || text.includes('victim') && text.includes('browser')
    );
    const hasXssNegation = (
      text.includes('script was sanitized') || text.includes('script was escaped') ||
      text.includes('did not execute') || text.includes('xss was blocked')
    );
    if (hasScriptPayload && hasStoredXssContext && hasBrowserExecution && !hasXssNegation) {
      result.verdict = 'Dangerous';
      result.classification = 'Stored Cross-Site Scripting (XSS)';
      result.severity = 'High';
      result.mitre_attack = ['T1190 — Exploit Public-Facing Application'];
      return result;
    }

    // JWT validation/auth bypass: modified token/claim is accepted because signature validation fails.
    const hasJwtContext = text.includes('jwt') || text.includes('json web token');
    const hasJwtTampering = text.includes('modified') || text.includes('forged') || text.includes('role claim') || text.includes('role":"admin');
    const hasJwtValidationFailure = (
      text.includes('without correctly validating') || text.includes('failed to validate') ||
      text.includes('failed to verify') || text.includes('without verifying') || text.includes('signature validation')
    );
    const hasUnauthorizedPrivilege = (
      text.includes('administrator-only') || text.includes('admin-only') || text.includes('granted administrator') ||
      text.includes('unauthorized admin') || text.includes('role":"admin')
    );
    const hasJwtRejection = text.includes('token was rejected') || text.includes('signature rejected') || text.includes('access denied');
    if (hasJwtContext && hasJwtTampering && hasJwtValidationFailure && hasUnauthorizedPrivilege && !hasJwtRejection) {
      result.verdict = 'Dangerous';
      result.classification = 'JWT Authentication / Authorization Bypass';
      result.severity = 'Critical';
      result.mitre_attack = ['T1078 — Valid Accounts'];
      return result;
    }

    // IDOR/BOLA: an authenticated principal changes an object identifier and receives another principal's protected object.
    const hasIdorContext = (
      text.includes('idor') || text.includes('insecure direct object reference') || text.includes('broken object level authorization') ||
      text.includes('changed an api request') || text.includes('modified the account id') || text.includes('changed the account id')
    );
    const hasCrossObjectAccess = (
      text.includes("another user's") || text.includes('another user') || text.includes('other account') ||
      text.includes('account 105') || text.includes('belonging to another')
    );
    const hasAuthorizationFailure = (
      text.includes('without checking') || text.includes('no authorization check') || text.includes('bypass authorization') ||
      text.includes('not authorized') || text.includes('unauthorized data access')
    );
    const hasIdorNegation = text.includes('request was denied') || text.includes('authorization check blocked') || text.includes('403 forbidden');
    if (hasIdorContext && hasCrossObjectAccess && hasAuthorizationFailure && !hasIdorNegation) {
      result.verdict = 'Dangerous';
      result.classification = 'IDOR / Broken Object Level Authorization';
      result.severity = 'High';
      result.mitre_attack = ['T1190 — Exploit Public-Facing Application'];
      return result;
    }

    // Compromised cloud credentials: exposed service-account credentials are actually used for unauthorized cloud access.
    const hasCloudCredential = (
      text.includes('service-account credential') || text.includes('service account credential') ||
      text.includes('temporary service-account') || text.includes('temporary service account')
    );
    const hasCredentialUse = (
      text.includes('were used') || text.includes('used from') || text.includes('authenticate successfully') ||
      text.includes('successful authentication')
    );
    const hasCloudResourceAccess = (
      text.includes('cloud storage') || text.includes('cloud resource') || text.includes('bucket') ||
      text.includes('enumerate') || text.includes('enumerating')
    );
    const hasCloudUseNegation = text.includes('credentials were not used') || text.includes('authentication failed') || text.includes('access was denied');
    if (hasCloudCredential && hasCredentialUse && hasCloudResourceAccess && !hasCloudUseNegation) {
      result.verdict = 'Dangerous';
      result.classification = 'Compromised Cloud Credentials / Unauthorized Cloud Access';
      result.severity = 'High';
      result.mitre_attack = ['T1078.004 — Valid Accounts: Cloud Accounts'];
      return result;
    }
  }
  ...
  if (mode === 'analyst') {
    const hasExplicitWebShellNegation = (
      /\bno\s+(?:evidence\s+of\s+)?web\s*shell\b/.test(text) ||
      /\bwithout\s+(?:a\s+)?web\s*shell\b/.test(text) ||
      /\bweb\s*shell\s+(?:was\s+)?not\s+(?:uploaded|present|found|observed)\b/.test(text)
    );
    const hasWebShellUpload = !hasExplicitWebShellNegation && (
  ...
  if (mode === 'analyst') {
    // Evaluate macro/Office parent evidence only in affirmative clauses. This prevents
    // phrases such as "no Office document or macro involved" from creating macro evidence.
    const macroParentEvidenceText = text.replace(
      /\b(?:no\s+evidence\s+of|no|without)\b[^.!?\n]*/g,
      ''
    );
    const hasMacroParent = (
      macroParentEvidenceText.includes('winword.exe') || macroParentEvidenceText.includes('excel.exe') || macroParentEvidenceText.includes('powerpnt.exe') ||
      macroParentEvidenceText.includes('microsoft word') || macroParentEvidenceText.includes('word spawned') ||
      macroParentEvidenceText.includes('.docm') || macroParentEvidenceText.includes('.xlsm') ||
      (macroParentEvidenceText.includes('.docx') && macroParentEvidenceText.includes('macro')) ||
      macroParentEvidenceText.includes('vba macro') || macroParentEvidenceText.includes('office macro') || macroParentEvidenceText.includes('macro')
    );
    const hasMaliciousChildProcess = (
      text.includes('certutil') || text.includes('mshta') || text.includes('wscript') ||
      text.includes('cscript') || text.includes('bitsadmin') || text.includes('powershell') ||
    ...
    if (hasMacroParent && hasMaliciousChildProcess && hasMacroDownloadBehavior) {
      ...
      // Ignore explicitly negated impact lists (for example, "no evidence of ... ransomware or exfiltration").
      const macroImpactEvidenceText = text.replace(
        /\b(?:no\s+evidence\s+of|no|without)\b[^.!?\n]*/g,
        ''
      );
      const hasConfirmedCriticalMacroImpact = (
        macroImpactEvidenceText.includes('domain controller') || macroImpactEvidenceText.includes('exfiltrat') ||
        macroImpactEvidenceText.includes('ransomware') || macroImpactEvidenceText.includes('privilege escalation')
      );
  ...
  if (mode === 'analyst' && !docDetection.isDocumentation) {
  ...
  if (mode === 'analyst' && !docDetection.isDocumentation) {
    ...
    const hasExplicitDomainControllerNegation = (
      /\bno\s+(?:domain controller|dc)\s+(?:was\s+)?(?:accessed|targeted|contacted|reached|involved|compromised)\b/.test(text) ||
      /\bwithout\s+(?:access(?:ing)?|targeting|contacting|reaching|compromising)\s+(?:a\s+|the\s+)?(?:domain controller|dc)\b/.test(text) ||
      /\bdid\s+not\s+(?:access|target|contact|reach|compromise)\s+(?:a\s+|the\s+)?(?:domain controller|dc)\b/.test(text)
    );
    const isDomainControllerTarget = !hasExplicitDomainControllerNegation && (
  ...
  if (isMaliciousObfuscatedPs && !docDetection.isDocumentation) {
    if (mode === 'everyday') {
      result.verdict = 'Dangerous';
    } else if (mode === 'analyst') {
      result.verdict = 'Dangerous';
  ...
  // Final consistency validation safeguard:
  // If reasoning describes benign documentation / no malicious indicators, downgrade elevated verdicts.
  ...
  const finalBenignPhrase = (
    finalReasoning.includes('no malicious indicators') ||
    finalReasoning.includes('benign documentation') ||
    finalReasoning.includes('standard project documentation') ||
    finalReasoning.includes('no evidence of phishing')
  );

  const finalHighOrCritical = (
    String(result.severity || '').toLowerCase() === 'high' ||
    String(result.severity || '').toLowerCase() === 'critical'
  );

  const finalDangerous = (
    String(result.verdict || '').toLowerCase() === 'dangerous'
  );

  if (finalBenignPhrase && (finalDangerous || finalHighOrCritical)) {
    result.classification = 'Documentation';
    result.severity = 'Low';
    result.verdict = 'Safe';
    result.confidence = 'High';
    ...
    result.recommended_action = 'No action required; this documentation content is benign.';
    result.analysis_scope = "This document was analysed as static documentation.\nThe analysis examined readable content only.\nDocumentation may reference security concepts,\nlogin pages,\nor attack terminology without being malicious.";
  } else if (result.classification === 'Documentation' || (docDetection.isDocumentation && String(result.verdict || '').toLowerCase() === 'safe')) {
    result.classification = 'Documentation';
    result.severity = 'Low';
    result.confidence = 'High';
    result.mitre_attack = [];
...
function buildPrompt(mode, text, hasImage) {
  ...
  const docDetection = detectDocumentation(text || '');
...
Return your response ONLY as a valid JSON object matching the following structure:
{
  ...
  "next_steps": ["Concrete next step 1", "Concrete next step 2"]
    ...
    if (docDetection.isDocumentation) {
      prompt += `\n\nDOCUMENTATION GUIDANCE:\n${DOCUMENTATION_INSTRUCTIONS}\n\nFor benign software documentation, return verdict 'Safe'.`;
    }
    if (text) {
...
- demonstrated server-side request forgery reaching an internal/cloud metadata service and returning protected metadata:
  "Server-Side Request Forgery (SSRF)"

- successful path traversal/arbitrary local file read without OS command execution:
  "Path Traversal / Arbitrary File Read"

- persisted attacker-controlled script that executes in another user's browser:
  "Stored Cross-Site Scripting (XSS)"

- modified/forged JWT accepted because cryptographic signature or authorization validation failed:
  "JWT Authentication / Authorization Bypass"

- authenticated user changes an object identifier and receives another user's protected object without authorization:
  "IDOR / Broken Object Level Authorization"

- exposed service-account/cloud credentials are actually used for unauthorized cloud authentication or resource access:
  "Compromised Cloud Credentials / Unauthorized Cloud Access"
...
Return your response ONLY as a valid JSON object matching the following structure:
{
  ...
  "unknowns": "Important missing information, or None identified"
    ...
    if (docDetection.isDocumentation) {
      prompt += `\n\nDOCUMENTATION GUIDANCE:\n${DOCUMENTATION_INSTRUCTIONS}\n\nFor benign software documentation, classify as 'Documentation' with verdict 'Safe' and severity 'Low'.`;
    }
    if (text) {
      prompt += `\n\n<user_submitted_content>\n${text}\n</user_submitted_content>`;
    }
...
module.exports = { analyzeLucidContent, detectDocumentation };
